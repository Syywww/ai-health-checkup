            ...
            <li v-for="(dayobject, index) in days" :key="index">
              ...
              <div class="everyday" v-else>
                ...
                <template v-for="(obj, idx) in leftobj" :key="idx">
                  <div v-if="obj.date === dayobject.day.getDate()"
                      :class="obj.number > obj.reservations ? 'usual' : 'fulled'">
...
import {
  ...
  addOrderSetting,
  uploadOrderSettingExcel
} from '@/api/reservation/setting'
        ...
        try {
          ...
          if (existing) {
            ...
            res = await editNumberByOrderDate({
              id: existing.id,        // 必须传 id
              number: number,
              orderDate: clickDateStr // 完整日期
            })
          } else {
